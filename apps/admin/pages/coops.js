...
async function loadCoops() {
    ...
    document.getElementById('coopsTable').innerHTML = coops.map(function(o) {
        ...
        if (o.status === 'pending') { actions += '<button class="btn-sm btn-success" onclick="validateOrg(\'' + o.id + '\')"><i class="fas fa-check"></i></button><button class="btn-sm btn-suspend" onclick="rejectOrg(\'' + o.id + '\')"><i class="fas fa-times"></i></button>'; }
        else if (o.status === 'active') { actions += '<button class="btn-sm btn-suspend" onclick="toggleOrgStatus(\'' + o.id + '\',\'active\')"><i class="fas fa-ban"></i></button>'; }
        else if (o.status === 'suspended') { actions += '<button class="btn-sm btn-success" onclick="toggleOrgStatus(\'' + o.id + '\',\'suspended\')"><i class="fas fa-check"></i></button>'; }
