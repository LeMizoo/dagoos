...
async function loadCodes() {
    ...
    if (tbody) tbody.innerHTML = myDrivers.map(function(d) {
        return '<tr><td><strong>' + (d.user ? d.user.name : 'N/A') + '</strong></td><td>' + (d.user ? d.user.phone || 'N/A' : 'N/A') + '</td><td><code style="font-size:16px;letter-spacing:2px;">' + d.driverCode + '</code></td><td><span class="badge badge-success">Actif</span></td><td><button class="btn-sm btn-edit">Nouveau code</button></td></tr>';
    }).join('') || '<tr><td colspan="5">Aucun chauffeur</td></tr>';
}
function renouvelerCodes() { if (confirm('Renouveler tous les codes ?')) { alert('Codes renouvelés !'); loadCodes(); } }
