function init_settings() {
    ...
    main.innerHTML = 
        ...
        '<div style="display:flex;gap:0;margin-bottom:24px;background:var(--card);border-radius:14px;overflow:hidden;">' +
            ...
            '<button class="settings-tab" id="tab-LIVRAISONS" onclick="switchTab(\'LIVRAISONS\')" style="flex:1;padding:16px;border:none;cursor:pointer;font-weight:600;">Livraisons</button>' +
        ...
        '<button onclick="saveSettings()" style="width:100%;padding:16px;background:#27AE60;color:white;border:none;border-radius:12px;font-size:16px;font-weight:700;cursor:pointer;margin-top:24px;"><i class="fas fa-save"></i> Enregistrer</button>' +
        '<style>.settings-tab.active{background:#27AE60!important;color:white!important;}.settings-tab:not(.active):hover{background:var(--border);}</style>';
    switchTab('PROFIL');
}
...
function switchTab(type) {
    ...
    else if (type === 'LIVRAISONS') loadLivraisonsTab();
...
async function loadProfilTab() {
    ...
    var html = '<div class="card" style="padding:24px;"><h3>Profil de la coopérative</h3>';
    ...
    if (org.plan === 'Premium' || org.plan === 'Standard') {
        html += '<div style="background:#D1FAE5;padding:16px;border-radius:10px;text-align:center;margin-top:12px;">🌐 Page vitrine active: <a href="https://dago-coop.pages.dev/' + (org.slug || '') + '" target="_blank">' + (org.slug || '') + '</a></div>';
    }
    html += '</div>';
...
async function loadPlansTab() {
    ...
    var fleetPlans = (plans || []).filter(function(p) { return p.type === 'COOPERATIVE'; });
...
function loadLivraisonsTab() {
    ...
    types.forEach(function(t, i) {
        html += '<div style="display:flex;align-items:center;justify-content:space-between;padding:14px;border:1px solid var(--border);border-radius:10px;margin-top:8px;">' +
            ...
            '<div style="text-align:right;"><span style="font-size:12px;color:#27AE60;">' + t.prix + '</span><br>' +
...
function loadLandingTab() {
    ...
    var html = '<div class="card" style="padding:24px;"><h3>Contenu de la landing page</h3><p style="color:var(--text2);">Modifiez les textes sur dago-coop.pages.dev</p>';
