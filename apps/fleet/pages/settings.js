function init_settings() {
    ...
    main.innerHTML = '<div class="topbar"><h1><i class="fas fa-cog"></i> Paramètres</h1></div>' +
...
async function loadSettingsData() {
    try {
        ...
        var myVehicles = vehicles.filter(function(v) { return v.driverId && myDrivers.find(function(d) { return d.id === v.driverId; }); });
        
        // Profil
        var html = '<div style="display:flex;align-items:center;gap:16px;margin-bottom:20px;">' +
            '<img src="' + (org.logo || 'https://dago-mobility.pages.dev/assets/logo/b-trans.png') + '" style="width:64px;height:64px;border-radius:14px;object-fit:cover;">' +
        ...
        if (org.plan === 'Premium' || org.plan === 'Standard') {
            ...
            vitrineHTML = '<div style="background:#D1FAE5;border-radius:12px;padding:20px;text-align:center;">' +
                ...
                '<p style="font-size:11px;color:var(--text2);">Modifiez votre logo et description ci-dessus pour personnaliser votre page.</p></div>';
        } else {
            vitrineHTML = '<div style="background:#FEF3C7;border-radius:12px;padding:20px;text-align:center;">' +
                ...
                '<p style="color:#92400E;">Contactez votre administrateur pour upgrader votre plan.</p></div>';
        }
