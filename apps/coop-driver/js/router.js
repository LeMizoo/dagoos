...
function getHeaderHTML() {
  ...
  var statusColor = pointageStatut === 'PRESENT' ? 'var(--success-fg)' : pointageStatut === 'PAUSE' ? 'var(--warning-fg)' : 'var(--error-fg)';
  var plate = window.currentVehicle ? window.currentVehicle.plate : '';
  
  return '<div style="background:var(--bg-surface);padding:10px 14px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:100;border-bottom:1px solid var(--border);">' +
    '<div style="display:flex;align-items:center;gap:8px;">' +
      '<img src="' + DAGOOS_CONFIG.logoUrl + '" style="width:36px;height:36px;object-fit:contain;border-radius:8px;">' +
      '<div>' +
        '<div style="font-size:14px;font-weight:700;color:var(--text-primary);">' + (user.name || 'Chauffeur') + '</div>' +
        '<div style="font-size:10px;color:#94A3B8;display:flex;gap:6px;align-items:center;flex-wrap:wrap;">' +
          (window.DAGOOS_DRIVER_CONTEXT && window.DAGOOS_DRIVER_CONTEXT.isCoop ? '<span style="padding:2px 6px;border-radius:20px;font-size:9px;background:var(--bg-soft);color:var(--success-fg);">🏢 Coopérative</span>' : '<span style="padding:2px 6px;border-radius:20px;font-size:9px;background:var(--bg-soft);color:var(--text-secondary);">🚛 Flotte</span>') +
          '<span style="padding:2px 6px;border-radius:20px;font-size:9px;background:' + statusColor + ';color:#fff;">' + statusLabel + '</span>' +
          (plate ? '<span style="padding:2px 6px;border-radius:20px;font-size:9px;background:var(--bg-soft);color:var(--text-secondary);">🏍️ ' + plate + '</span>' : '<span style="padding:2px 6px;border-radius:20px;font-size:9px;background:var(--error-fg);color:var(--text-on-accent);">⚠️ Sans moto</span>') +
          '<span style="color:#94A3B8;">' + (user.driverCode || '') + '</span>' +
        '</div>' +
      '</div>' +
    '</div>' +
    '<div style="display:flex;gap:4px;">' +
      '<button onclick="loadPage(\'notifications\')" style="background:var(--bg-soft);border:none;width:32px;height:32px;border-radius:50%;color:var(--accent);cursor:pointer;font-size:14px;">🔔</button>' +
      '<button onclick="loadPage(\'profil\')" style="background:var(--bg-soft);border:none;width:32px;height:32px;border-radius:50%;color:var(--accent);cursor:pointer;font-size:14px;">📶</button>' +
      '<button onclick="logout()" style="background:var(--error-bg);border:none;width:32px;height:32px;border-radius:50%;color:var(--error-fg);cursor:pointer;font-size:16px;" aria-label="Déconnexion" title="Déconnexion">⏻</button>' +
    '</div>' +
  '</div>';
}
