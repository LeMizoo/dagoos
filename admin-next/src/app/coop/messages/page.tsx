        ...
        <div className="w-80 bg-white rounded-xl shadow-sm border flex flex-col flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? <div className="text-center py-8 text-gray-400">⏳ Chargement...</div> :
             ...
             filtered.map(m => (
              <div key={m.id} onClick={() => handleSelect(m.id)} className={`p-3 border-b cursor-pointer hover:bg-gray-50 ${selected === m.id ? 'bg-emerald-50 border-l-2 border-l-emerald-500' : ''}`}>
                <div className="flex items-center gap-2 mb-1">{m.read ? <MailOpen size={14} className="text-gray-400" /> : <Mail size={14} className="text-emerald-500" />}<span className={`text-sm ${!m.read ? 'font-semibold' : ''}`}>{m.organization?.name || 'Inconnu'}</span></div>
          ...
          {selectedMsg ? (
            <>
              ...
              <div className="p-4 border-t flex gap-2">
                ...
                <button onClick={handleSend} disabled={sending || !reply.trim()} className="bg-emerald-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm hover:bg-emerald-700 disabled:opacity-50"><Send size={14} /> {sending ? 'Envoi...' : 'Envoyer'}</button>
              </div>
            </>
          ) : (
