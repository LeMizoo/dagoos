...
export default function CoopDriversPage() {
  ...
  const [form, setForm] = useState({ driverCode: '', pin: '', status: 'active', fullName: '' });
  ...
  async function fetchDrivers() {
    try {
      ...
      const allDrivers = Array.isArray(data) ? data : [];
      if (currentUser?.organizationId) {
        setDrivers(allDrivers.filter((d: Driver) => d.organizationId === currentUser.organizationId));
      } else {
  ...
  function openCreate() {
    setEditItem(null);
    setSubmitError('');
    setForm({ driverCode: '', pin: '', status: 'active', fullName: '' });
    setModalOpen(true);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const payload: any = {
        ...
        pin: form.pin,
        status: form.status,
        user: { name: form.fullName }  // toujours envoyer le nom
      };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={openCreate} className="bg-emerald-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-emerald-700 transition text-sm">
        ...
        <div className="p-4 border-b">
          <div className="relative max-w-sm">
            ...
            <input type="text" placeholder="Rechercher..." value={search} onChange={e => setSearch(e.target.value)}
              className="pl-9 pr-4 py-2 border rounded-lg w-full text-sm focus:ring-2 focus:ring-emerald-500 focus:border-transparent" />
          </div>
        </div>
                  ...
                  <tr key={d.id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button onClick={() => openEdit(d)} className="p-1 hover:bg-emerald-100 rounded text-emerald-600"><Edit size={14} /></button>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Modifier le chauffeur' : 'Nouveau chauffeur'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Code chauffeur (toujours présent) */}
          <div>
          ...
          <div className="flex gap-2 pt-2">
            <button type="button" onClick={() => setModalOpen(false)} className="flex-1 px-4 py-2 border rounded-lg text-sm hover:bg-gray-50">
              Annuler
            </button>
            <button type="submit" disabled={saving} className="flex-1 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm hover:bg-emerald-700 disabled:bg-gray-400">
