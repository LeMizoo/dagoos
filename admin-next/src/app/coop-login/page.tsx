...
function CoopLoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError('');
    const res = await apiFetch('/api/auth/coop-login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-900 to-green-700">
      <div className="bg-white rounded-2xl p-10 w-full max-w-md shadow-2xl">
        <div className="text-center mb-8"><div className="w-16 h-16 bg-emerald-100 rounded-2xl flex items-center justify-center mx-auto mb-4"><Building2 size={32} className="text-emerald-600" /></div><h1 className="text-2xl font-bold text-gray-800">Dagoo Coop</h1><p className="text-gray-500 text-sm mt-1">Gérez votre coopérative de chauffeurs</p><span className="inline-block bg-emerald-100 text-emerald-700 text-xs px-3 py-1 rounded-full mt-3">🏢 Coopérative</span></div>
...
export default function CoopLoginPage() { return <Suspense fallback={<div className="min-h-screen flex items-center justify-center">Chargement...</div>}><CoopLoginForm /></Suspense>; }
