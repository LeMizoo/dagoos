  ...
  useEffect(() => {
    apiFetch('/plans')
      ...
      .then(plans => {
        ...
        setFleetPlans(prev => {
          const next = { ...prev };
          plans.filter(p => p.type === 'FLEET_MANAGER').forEach(p => {
            const key = p.name.toLowerCase() as PlanKey;
            if (key in next) next[key] = { ...next[key], ...p };
          });
          return next;
        });
        ...
        setCoopPlans(prev => {
          const next = { ...prev };
          plans.filter(p => p.type === 'COOPERATIVE').forEach(p => {
            const key = p.name.toLowerCase() as PlanKey;
            if (key in next) next[key] = { ...next[key], ...p };
          });
          return next;
                    ...
                    <div key={planKey} className={`bg-white dark:bg-gray-750 rounded-xl border ${planColors[planKey]} dark:border-gray-600 p-4 transition-all`}>
                      ...
                      {isEditing ? (
                        <div className="flex gap-1.5">
                          <button onClick={() => {
                            apiFetch(`/plans/${currentPlans[planKey].id}`, {
                              method: 'PUT',
                              body: JSON.stringify({ price: editPrice }),
                            })
                              .then(r => {
                                if (!r.ok) throw new Error('Erreur sauvegarde plan');
                                return r.json();
                              })
                              .then(updated => {
                                setCurrentPlans(prev => ({
                                  ...prev,
                                  [planKey]: { ...prev[planKey], ...updated },
                                }));
                                setEditPlan(null);
                              })
                              .catch(err => console.error(err));
                          }} className="flex-1 py-1.5 text-xs bg-green-600 text-white rounded-lg">Valider</button>
