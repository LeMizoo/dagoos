    ...
    <div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? <div className="p-8 text-center text-gray-400">Chargement...</div> :
         drivers.length === 0 ? <div className="p-12 text-center text-gray-400"><Users size={40} className="mx-auto mb-3 opacity-50" /><p>Aucun chauffeur.</p></div> :
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingDriver ? 'Modifier' : 'Ajouter'}>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <input type="text" placeholder="Code" value={formData.code} onChange={e => setFormData({...formData, code: e.target.value})} className="w-full px-3 py-2.5 border rounded-lg text-sm" />
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={saving} className="px-4 py-2 text-sm bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50">{saving ? '...' : editingDriver ? 'Enregistrer' : 'Ajouter'}</button>
          </div>
        </form>
      </Modal>
