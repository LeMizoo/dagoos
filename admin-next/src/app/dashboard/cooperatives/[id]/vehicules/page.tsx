...
export default function CoopVehiclesPage() {
  ...
  const openCreate = () => { setEditingVehicle(null); setFormData({ plate: '', model: '', brand: '', year: 2024, status: 'active' }); setModalOpen(true); };
  const openEdit = (v: Vehicle) => { setEditingVehicle(v); setFormData({ plate: v.plate, model: v.model, brand: v.brand || '', year: v.year || 2024, status: v.status }); setModalOpen(true); };
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        ...
        <button onClick={openCreate} className="flex items-center gap-2 bg-emerald-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-emerald-700 transition shrink-0">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? <div className="p-8 text-center text-gray-400">Chargement...</div> :
         vehicles.length === 0 ? <div className="p-12 text-center text-gray-400"><Car size={40} className="mx-auto mb-3 opacity-50" /><p>Aucun véhicule.</p></div> :
         <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-gray-50 text-left text-gray-500"><tr><th className="px-4 py-3">Plaque</th><th className="px-4 py-3">Modèle</th><th className="px-4 py-3">Année</th><th className="px-4 py-3">Statut</th><th className="px-4 py-3 text-right">Actions</th></tr></thead>
         <tbody>{vehicles.map(v => (
           <tr key={v.id} className="border-t hover:bg-gray-50">
             <td className="px-4 py-3 font-medium">{v.plate}</td>
             <td className="px-4 py-3">{v.brand} {v.model}</td>
             <td className="px-4 py-3">{v.year}</td>
             <td className="px-4 py-3"><span className={`text-xs px-2 py-0.5 rounded-full ${v.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{v.status}</span></td>
             <td className="px-4 py-3 text-right">
               <button onClick={() => openEdit(v)} className="p-1.5 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg"><Pencil size={16} /></button>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingVehicle ? 'Modifier' : 'Ajouter'}>
