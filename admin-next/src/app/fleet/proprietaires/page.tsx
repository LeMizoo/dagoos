    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{filtered.length} propriétaire{filtered.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? <div className="col-span-full text-center py-8 text-gray-400">Chargement...</div> :
         filtered.length === 0 ? <div className="col-span-full text-center py-8 text-gray-400">Aucun propriétaire</div> :
         filtered.map(p => (
          <div key={p.id} className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition">
            ...
            <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400 mb-4">
              ...
              <div className="flex items-center gap-4 mt-2">
                <span className="flex items-center gap-1 text-xs"><Car size={14} /> {p._count?.vehicles || p.vehicles?.length || 0} véhicules</span>
              </div>
            </div>
            <Link href={`/fleet/proprietaires/${p.id}`} 
              className="block w-full text-center py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-700 rounded-lg text-sm hover:bg-blue-100 transition">
              Voir les véhicules →
            </Link>
          </div>
        ))}
