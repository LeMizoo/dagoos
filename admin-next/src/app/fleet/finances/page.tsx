      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl p-5 shadow-sm border">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center"><TrendingUp size={20} className="text-green-600" /></div>
            <span className="text-sm text-gray-500">Chiffre d&apos;affaires</span>
          </div>
          <div className="text-2xl font-bold text-green-600">{loading ? '-' : totalCA.toLocaleString() + ' Ar'}</div>
        </div>
        <div className="bg-white rounded-xl p-5 shadow-sm border">
          <div className="flex items-center gap-3 mb-2">
          ...
          <div className="text-2xl font-bold text-red-600">{loading ? '-' : totalCommission.toLocaleString() + ' Ar'}</div>
        </div>
        <div className="bg-white rounded-xl p-5 shadow-sm border">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm text-gray-500">Net chauffeurs</span>
          </div>
          <div className="text-2xl font-bold text-blue-600">{loading ? '-' : totalNet.toLocaleString() + ' Ar'}</div>
        </div>
      </div>
              ...
              {loading ? <tr><td colSpan={7} className="text-center py-8 text-gray-400">Chargement...</td></tr> :
               ...
               filtered.map(c => (
                <tr key={c.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">{c.vehicle?.plate || '-'} {c.vehicle?.model ? `(${c.vehicle.model})` : ''}</td>
                  ...
                  <td className="px-4 py-3 font-medium text-blue-600">{((c.price || 0) - (c.commission || 0)).toLocaleString()} Ar</td>
                </tr>
              ))}
            </tbody>
          </table>
