...
export default function FleetDepensesPage() {
  ...
  const totalAujourdhui = depenses
    .filter(d => d.date === new Date().toISOString().split('T')[0])
    .reduce((s, d) => s + d.montant, 0);
  const totalSemaine = depenses
    .filter(d => {
      const now = new Date();
  ...
  function handleAdd() {
    ...
    setForm({ categorie: 'Carburant', description: '', montant: 0 });
    setModalOpen(false);
  }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setModalOpen(true)} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-700 transition text-sm">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl p-4 shadow-sm border text-center">
          <div className="text-2xl font-bold text-gray-800">{totalAujourdhui.toLocaleString()} Ar</div>
          <div className="text-xs text-gray-500">Aujourd&apos;hui</div>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-sm border text-center">
          <div className="text-2xl font-bold text-gray-800">{totalSemaine.toLocaleString()} Ar</div>
          <div className="text-xs text-gray-500">Cette semaine</div>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-sm border text-center">
          <div className="text-2xl font-bold text-gray-800">{depenses.reduce((s, d) => s + d.montant, 0).toLocaleString()} Ar</div>
        ...
        <div className="p-4 border-b flex justify-between items-center">
          ...
          <div className="relative max-w-xs">
            ...
            <input type="text" placeholder="Rechercher..." value={search} onChange={e => setSearch(e.target.value)}
              className="pl-9 pr-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500" />
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[600px]">
            <thead className="bg-gray-50 text-left text-gray-500">
              <tr><th className="px-4 py-3">Date</th><th className="px-4 py-3">Catégorie</th><th className="px-4 py-3">Description</th><th className="px-4 py-3">Montant</th><th className="px-4 py-3">Actions</th></tr>
            </thead>
            <tbody>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nouvelle dépense">
        <div className="space-y-4">
          <div>
            ...
            <select value={form.categorie} onChange={e => setForm({ ...form, categorie: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-sm">
          ...
          <div>
            ...
            <input type="number" value={form.montant} onChange={e => setForm({ ...form, montant: parseInt(e.target.value) || 0 })} className="w-full px-3 py-2 border rounded-lg text-sm" />
          </div>
          <div className="flex gap-2 pt-2">
            <button onClick={() => setModalOpen(false)} className="flex-1 px-4 py-2 border rounded-lg text-sm">Annuler</button>
            <button onClick={handleAdd} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">Ajouter</button>
          </div>
        </div>
      </Modal>
    </div>
