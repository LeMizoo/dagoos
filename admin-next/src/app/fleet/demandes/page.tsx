    ...
    <div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {['all', 'NEW', 'IN_PROGRESS', 'ACCEPTED', 'REJECTED'].map(f => (
          <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${filter === f ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
            {f === 'all' ? 'Toutes' : f}
          </button>
        ))}
      </div>
