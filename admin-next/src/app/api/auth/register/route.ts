...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await fetch('https://dagoos-api.onrender.com/api/auth/register', {
      ...
      body: JSON.stringify({ name, email, password, role, organizationName, planId }),
    });
    const data = await response.json();
    if (!response.ok) {
      return NextResponse.json({ error: data.error || 'Erreur lors de l\'inscription' }, { status: response.status });
    }
