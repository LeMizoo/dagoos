...
const menus: Record<string, any> = {
  admin: [
    { section: 'Principal', items: [{ href: '/dashboard', icon: LayoutDashboard, label: 'Tableau de bord' }, { href: '/dashboard/flottes', icon: Truck, label: 'Flottes' }, { href: '/dashboard/cooperatives', icon: Building2, label: 'Coopératives' }, { href: '/dashboard/finances', icon: DollarSign, label: 'Finances' }] },
  ...
  fleet: [
    ...
    { section: 'Autres', items: [{ href: '/fleet/messages', icon: MessageSquare, label: 'Messages' }, { href: '/fleet/notifications', icon: Bell, label: 'Notifications' }, { href: '/fleet/rapports', icon: FileText, label: 'Rapports' }, { href: '/fleet/profil', icon: User, label: 'Profil' }, { href: '/fleet/settings', icon: Settings, label: 'Paramètres' }] },
  ],
  coop: [
        ...
        <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 lg:px-6 py-3 flex items-center justify-between sticky top-0 z-30">
          <div>
            <h1 className="text-lg font-bold text-gray-800 dark:text-white">
              {user?.organizationName || (app === 'admin' ? 'Tableau de Bord Admin' : app === 'fleet' ? 'Tableau de Bord Flotte' : 'Tableau de Bord Coopérative')}
            </h1>
              ...
              <>
                ...
                <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 z-50 overflow-hidden">
                  <div className="p-4 border-b border-gray-100 dark:border-gray-700">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">{user?.email || ''}</p>
                    <span className="inline-block mt-1 text-xs bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full capitalize">{user?.role || app}</span>
                  </div>
                  <div className="p-2">
                    <Link href={app === 'admin' ? '/dashboard/settings' : `/${app}/profil`} onClick={() => setShowUserMenu(false)}
