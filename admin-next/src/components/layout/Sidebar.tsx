  ...
  return (
    <aside className="w-60 bg-dark text-white fixed h-full flex flex-col">
      <div className="p-4 border-b border-gray-700">
        <h2 className="text-lg font-bold">DAG<span className="text-secondary">OO</span></h2>
        ...
        {menu.map(s => (
          <div key={s.section} className="mb-4">
            ...
            {s.items.map(item => { const Icon = item.icon; const isActive = pathname === item.href; return (
              <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-3 py-2 text-sm rounded-lg transition mb-0.5 ${isActive ? 'bg-primary text-white font-medium' : 'text-gray-300 hover:bg-white/10'}`}><Icon size={18} /><span>{item.label}</span></Link>
            );})}
          </div>
        ))}
      </nav>
      <div className="p-3 border-t border-gray-700">
        <button onClick={handleLogout} className="flex items-center gap-3 px-3 py-2 text-sm text-gray-300 hover:bg-red-500/20 hover:text-red-400 rounded-lg transition w-full"><LogOut size={18} /><span>Déconnexion</span></button>
      </div>
    </aside>
  );
}
