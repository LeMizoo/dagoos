        ...
        <div className="w-80 bg-white rounded-xl shadow-sm border flex flex-col flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? <div className="text-center py-8 text-gray-400">⏳ Chargement...</div> :
             ...
             messages.map(m => (
              <div key={m.id} onClick={() => setSelected(m.id)} className={`p-3 border-b cursor-pointer hover:bg-gray-50 ${selected === m.id ? 'bg-blue-50 border-l-2 border-l-blue-500' : ''}`}>
                <div className="flex items-center gap-2 mb-1">{m.read ? <MailOpen size={14} className="text-gray-400" /> : <Mail size={14} className="text-blue-500" />}<span className="text-sm font-medium">{m.organization?.name || 'Inconnu'}</span></div>
        ...
        <div className="flex-1 bg-white rounded-xl shadow-sm border flex flex-col">
          {selectedMsg ? (
            <>
              ...
              <div className="p-4 border-t flex gap-2"><input type="text" value={reply} onChange={e => setReply(e.target.value)} placeholder="Votre réponse..." className="flex-1 px-3 py-2 border rounded-lg text-sm" /><button className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm"><Send size={14} /> Envoyer</button></div>
            </>
          ) : (
