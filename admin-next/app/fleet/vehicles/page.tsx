...
import { Car, Plus, Search, AlertTriangle, CheckCircle, Wrench, Filter } from 'lucide-react';
...
export default function FleetVehicles() {
  ...
  async function load() {
    try {
      const r = await apiFetch('/api/proxy/vehicles');
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        <div className="p-4 border-b dark:border-gray-700 flex flex-wrap gap-3">
          ...
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)}
            ...
            {Object.entries(FLEET_VEHICLE_TYPES).map(([k, v]) => (
              ...
              {loading ? <tr><td colSpan={7} className="text-center py-8">Chargement...</td></tr> :
               ...
               filtered.map(v => (
                <tr key={v.id} className="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td className="px-4 py-3">{FLEET_VEHICLE_TYPES[v.type] || v.type || '-'}</td>
                  ...
                  <td className="px-4 py-3 text-gray-500">{v.proprietaire?.name || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
