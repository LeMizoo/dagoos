...
export default function CooperativesPage() {
  ...
  const fetchOrgs = async () => {
    ...
    try {
      ...
      setOrgs(arr.filter((o: any) => o.type === 'COOPERATIVE' || o.type === 'coop'));
      setError('');
    } catch (err: any) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {filteredOrgs.length} coopérative{filteredOrgs.length !== 1 ? 's' : ''} 
            {loading ? ' (chargement...)' : ''}
          </p>
        </div>
        <button
          onClick={openCreate}
          className="flex items-center gap-2 bg-emerald-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-emerald-700 transition shadow-sm shrink-0"
        >
          <Plus size={18} />
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition"
          />
        </div>
        <select
          value={planFilter}
          onChange={e => setPlanFilter(e.target.value)}
          className="px-3 py-2.5 bg-white border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
        >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-700 mb-1">Aucune coopérative trouvée</h3>
            <p className="text-sm text-gray-500 mb-4">
              {search || planFilter !== 'all' || statusFilter !== 'all' 
                ? 'Essayez de modifier vos filtres.' 
                : 'Créez votre première coopérative pour commencer.'}
            </p>
            {!search && planFilter === 'all' && statusFilter === 'all' && (
              <button onClick={openCreate} className="text-emerald-600 text-sm hover:underline">
                + Nouvelle coopérative
              </button>
            )}
          </div>
                ...
                {filteredOrgs.map(org => (
                  <tr key={org.id} className="border-t border-gray-50 hover:bg-gray-50/50 transition-colors">
                    <td className="px-4 py-3">
                      <Link href={`/dashboard/cooperatives/${org.id}`} className="flex items-center gap-2 group">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        ...
                        org.plan === 'STANDARD' ? 'bg-emerald-100 text-emerald-700' :
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Link
                          href={`/dashboard/cooperatives/${org.id}`}
      ...
      <Modal
        ...
        title={editingOrg ? 'Modifier la coopérative' : 'Nouvelle coopérative'}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex: Coopérative Tana"
              required
            />
          </div>
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none"
              placeholder="Ex: contact@coop.mg"
              required
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
              >
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-3 bg-red-50 rounded-lg">
            ...
            <p className="text-sm text-red-700">
              Êtes-vous sûr de vouloir supprimer la coopérative <strong>{deleteConfirm?.name}</strong> ? 
              Cette action est irréversible et supprimera tous les chauffeurs, véhicules, sociétés et données associés.
            </p>
          </div>
