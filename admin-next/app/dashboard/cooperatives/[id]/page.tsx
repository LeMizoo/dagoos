...
import { 
  Building2, Users, Car, FileCheck, Wrench, 
...
export default function CoopDetailPage() {
  ...
  if (error || !org) {
    return (
      <div className="p-12 text-center">
        ...
        <h2 className="text-lg font-semibold text-gray-700 mb-2">Coopérative introuvable</h2>
        <p className="text-sm text-gray-500 mb-4">{error || 'Cette coopérative n\'existe pas ou a été supprimée.'}</p>
        <Link href="/dashboard/cooperatives" className="text-emerald-600 hover:underline text-sm">
  ...
  const subMenu = [
    { href: `/dashboard/cooperatives/${org.id}/societes`, icon: Briefcase, label: 'Sociétés', desc: 'Gérer les sociétés membres' },
    { href: `/dashboard/cooperatives/${org.id}/vehicules`, icon: Car, label: 'Véhicules', desc: 'Gérer les véhicules' },
    { href: `/dashboard/cooperatives/${org.id}/chauffeurs`, icon: Users, label: 'Chauffeurs', desc: 'Gérer les chauffeurs' },
    { href: `/dashboard/cooperatives/${org.id}/contrats`, icon: FileCheck, label: 'Contrats', desc: 'Suivre les contrats' },
  ];
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
        <Link href="/dashboard/cooperatives" className="hover:text-emerald-600 transition-colors">Coopératives</Link>
        <span>/</span>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-emerald-100 mb-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-6 border-t border-gray-100">
          <QuickStat icon={Briefcase} label="Sociétés" value="-" color="emerald" />
          <QuickStat icon={Car} label="Véhicules" value={org._count?.vehicles || 0} color="blue" />
        ...
        {subMenu.map(item => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 hover:shadow-md hover:border-emerald-200 transition-all group"
            >
...
function QuickStat({ icon: Icon, label, value, color, isDate }: {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${colorMap[color] || colorMap.emerald}`}>
        <Icon size={18} />
      </div>
      <div>
